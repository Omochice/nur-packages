# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  cc-sdd = {
    pname = "cc-sdd";
    version = "3.0.1";
    src = fetchurl {
      url = "https://registry.npmjs.org/cc-sdd/-/cc-sdd-3.0.1.tgz";
      sha256 = "sha256-moksMsYo4UY0lG7hADRMY6MKtJ0W6WhcR/3vVZihNgA=";
    };
  };
  ccusage = {
    pname = "ccusage";
    version = "18.0.10";
    src = fetchurl {
      url = "https://registry.npmjs.org/ccusage/-/ccusage-18.0.10.tgz";
      sha256 = "sha256-YMJ8K2LmS0v0HeoRskfULZ7P7rVBpgYJRlDmZjndRiw=";
    };
  };
  claude-code = {
    pname = "claude-code";
    version = "2.1.101";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.101.tgz";
      sha256 = "sha256-wWXXLCpU0vHrm71uKrE2nV4Uo8Rqe8OweRIn7KoMFFk=";
    };
  };
  disable-checkout-persist-credentials = {
    pname = "disable-checkout-persist-credentials";
    version = "v1.0.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "disable-checkout-persist-credentials";
      rev = "v1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-TEjUf7Uc049g/ZRuKMtWYxod+ZN5dKabDINJgqXxkuk=";
    };
  };
  duckgo = {
    pname = "duckgo";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "sheepla";
      repo = "duckgo";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-+wJexVAKTqEOTyV+PcNcUe5r9Q4MpGx9EnuxiQHq0G0=";
    };
  };
  firge = {
    pname = "firge";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/Firge_v0.3.0.zip";
      sha256 = "sha256-yYb2IQAKZZgGTH4geDIu0CBAr094BNJYAz35SbNbBSc=";
    };
  };
  firge-nerd = {
    pname = "firge-nerd";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/FirgeNerd_v0.3.0.zip";
      sha256 = "sha256-VM12N4+8UCX0LUQdlcpuwdPsxCcOYQdViED+18BM/k8=";
    };
  };
  gh-dep = {
    pname = "gh-dep";
    version = "v0.11.2";
    src = fetchFromGitHub {
      owner = "jackchuka";
      repo = "gh-dep";
      rev = "v0.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-bKBMaXkNWxdJV3t5ny/bSQBF8DcgNyKguQtoheHHP2E=";
    };
  };
  gh-grep = {
    pname = "gh-grep";
    version = "v1.2.5";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "gh-grep";
      rev = "v1.2.5";
      fetchSubmodules = false;
      sha256 = "sha256-xCr68IMrGPIQr+RmAgNdgD+Bw0dQSSi9GcpoDy3q86U=";
    };
  };
  gh-triage = {
    pname = "gh-triage";
    version = "v0.11.0";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "gh-triage";
      rev = "v0.11.0";
      fetchSubmodules = false;
      sha256 = "sha256-aa98DfXxyOuk1Q7yYGKNqYqTa9tuNP57hYwNqIYxPeo=";
    };
  };
  ghalint = {
    pname = "ghalint";
    version = "v1.5.5";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghalint";
      rev = "v1.5.5";
      fetchSubmodules = false;
      sha256 = "sha256-xAXcvvSwcJjdG2BCItBLdsu6vZiID5FmRYYF9PZe1QE=";
    };
  };
  ghatm = {
    pname = "ghatm";
    version = "v1.0.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghatm";
      rev = "v1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-MX1VilyfxJ9GKIkDSGeAE007DyjPkWL5W4b08EqAyC4=";
    };
  };
  ghq = {
    pname = "ghq";
    version = "v1.10.1";
    src = fetchFromGitHub {
      owner = "x-motemen";
      repo = "ghq";
      rev = "v1.10.1";
      fetchSubmodules = false;
      sha256 = "sha256-SmcgBwd5k/lAv9bwYRpkIM0fil2ajSlT8zznP7bgpDk=";
    };
  };
  git-wt = {
    pname = "git-wt";
    version = "v0.26.2";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "git-wt";
      rev = "v0.26.2";
      fetchSubmodules = false;
      sha256 = "sha256-zAQxo9rgNq9L+NOMx4xS+h0oBGukZqfRg0Y3OYdelA0=";
    };
  };
  gitlab-ci-ls = {
    pname = "gitlab-ci-ls";
    version = "1.3.1";
    src = fetchFromGitHub {
      owner = "alesbrelih";
      repo = "gitlab-ci-ls";
      rev = "1.3.1";
      fetchSubmodules = false;
      sha256 = "sha256-ZZjx6VdBZuVuHl42n0iXZkvvUku2CN7x4JCZxZTMOMk=";
    };
  };
  gitlab-ci-verify = {
    pname = "gitlab-ci-verify";
    version = "v2.11.0";
    src = fetchFromGitHub {
      owner = "timo-reymann";
      repo = "gitlab-ci-verify";
      rev = "v2.11.0";
      fetchSubmodules = false;
      sha256 = "sha256-HmNyou/B0TFKz0CgnBNFfmgbHf37P6QN6QCRaH5KHjk=";
    };
  };
  ibm-plex-sans-jp = {
    pname = "ibm-plex-sans-jp";
    version = "@ibm/plex-sans-jp@3.0.0";
    src = fetchurl {
      url = "https://github.com/IBM/plex/releases/download/@ibm/plex-sans-jp@3.0.0/ibm-plex-sans-jp.zip";
      sha256 = "sha256-TBTEFVKTSwvJL8IWp2s2vcePdF377tERX5uLxAXuMU8=";
    };
  };
  lazygit = {
    pname = "lazygit";
    version = "v0.61.1";
    src = fetchFromGitHub {
      owner = "jesseduffield";
      repo = "lazygit";
      rev = "v0.61.1";
      fetchSubmodules = false;
      sha256 = "sha256-sFmOHsYs39i4mSH/CnetWFBQBviv5QsP0qfCSkaEJZE=";
    };
  };
  octocov = {
    pname = "octocov";
    version = "v0.75.6";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "octocov";
      rev = "v0.75.6";
      fetchSubmodules = false;
      sha256 = "sha256-tea20KoyIpJJCP/HGG5/h6Pa+BejFuXlMkYEGzf9SSA=";
    };
  };
  pinact = {
    pname = "pinact";
    version = "v3.9.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "pinact";
      rev = "v3.9.0";
      fetchSubmodules = false;
      sha256 = "sha256-FbqVf7ANY2wDrQCnDp3pVAT0PyMzwNgdusgNkaNAlKk=";
    };
  };
  roots = {
    pname = "roots";
    version = "v0.4.1";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "roots";
      rev = "v0.4.1";
      fetchSubmodules = false;
      sha256 = "sha256-ACMRfWY/lhc3C/KVhuUyS1rgkSHGWPxZrmYt+pXupJI=";
    };
  };
  slack-reminder = {
    pname = "slack-reminder";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "skanehira";
      repo = "slack-reminder";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-1MzRZ8E1wtwThnaXteMkAapX5kLFbEi3ix1csatSFF8=";
    };
  };
  sort-package-json = {
    pname = "sort-package-json";
    version = "v3.6.1";
    src = fetchFromGitHub {
      owner = "keithamus";
      repo = "sort-package-json";
      rev = "v3.6.1";
      fetchSubmodules = false;
      sha256 = "sha256-PGstuD9NNdY8dkrtLLZjGbUJ7fb4U5mPSFygnZcGi9k=";
    };
  };
  vim-startuptime = {
    pname = "vim-startuptime";
    version = "v1.3.2";
    src = fetchFromGitHub {
      owner = "rhysd";
      repo = "vim-startuptime";
      rev = "v1.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-d6AXTWTUawkBCXCvMs3C937qoRUZmy0qCFdSLcWh0BE=";
    };
  };
}
