# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  ccusage = {
    pname = "ccusage";
    version = "16.2.0";
    src = fetchurl {
      url = "https://registry.npmjs.org/ccusage/-/ccusage-16.2.0.tgz";
      sha256 = "sha256-/cOWG1VeyRGgEa8N0KNVrPkwtYd8D5ITp/Q0VTHYsF8=";
    };
  };
  claude-code = {
    pname = "claude-code";
    version = "1.0.95";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-1.0.95.tgz";
      sha256 = "sha256-sZ5eRafYzYUtcGeBON4fyjreLMcipeL/x4JNJSdchFg=";
    };
  };
  disable-checkout-persist-credentials = {
    pname = "disable-checkout-persist-credentials";
    version = "v0.1.3";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "disable-checkout-persist-credentials";
      rev = "v0.1.3";
      fetchSubmodules = false;
      sha256 = "sha256-CRngoxWKeiYvA6AM6o3M6t0AsWEvA+D+1d34/vT+n3Y=";
    };
  };
  duckgo = {
    pname = "duckgo";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "sheepla";
      repo = "duckgo";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-+wJexVAKTqEOTyV+PcNcUe5r9Q4MpGx9EnuxiQHq0G0=";
    };
  };
  firge = {
    pname = "firge";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/Firge_v0.3.0.zip";
      sha256 = "sha256-yYb2IQAKZZgGTH4geDIu0CBAr094BNJYAz35SbNbBSc=";
    };
  };
  firge-nerd = {
    pname = "firge-nerd";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/FirgeNerd_v0.3.0.zip";
      sha256 = "sha256-VM12N4+8UCX0LUQdlcpuwdPsxCcOYQdViED+18BM/k8=";
    };
  };
  ghalint = {
    pname = "ghalint";
    version = "v1.5.3";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghalint";
      rev = "v1.5.3";
      fetchSubmodules = false;
      sha256 = "sha256-zll71vSqSKIij/TUi4LnGtVqmLhK9UViFDkpnuEvmz8=";
    };
  };
  ghatm = {
    pname = "ghatm";
    version = "v0.3.7";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghatm";
      rev = "v0.3.7";
      fetchSubmodules = false;
      sha256 = "sha256-t8DjUHsbU1ovBaB2bINEG7gpm1MFQCnoanunfrrTKWU=";
    };
  };
  ghq = {
    pname = "ghq";
    version = "v1.8.0";
    src = fetchFromGitHub {
      owner = "x-motemen";
      repo = "ghq";
      rev = "v1.8.0";
      fetchSubmodules = false;
      sha256 = "sha256-5BN96/RShfJpkfpJe0qrZVDuyFoAV9kgCiBv4REY/5Y=";
    };
  };
  gitlab-ci-ls = {
    pname = "gitlab-ci-ls";
    version = "1.1.11";
    src = fetchFromGitHub {
      owner = "alesbrelih";
      repo = "gitlab-ci-ls";
      rev = "1.1.11";
      fetchSubmodules = false;
      sha256 = "sha256-PQYWls0b6HndklDI/1cUq6+r99GeSfRLGxMKm0HgQqY=";
    };
  };
  gitlab-ci-verify = {
    pname = "gitlab-ci-verify";
    version = "v2.4.0";
    src = fetchFromGitHub {
      owner = "timo-reymann";
      repo = "gitlab-ci-verify";
      rev = "v2.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-uaeyq3q47CdfDjJ8WwmIO/HBdg/q0gJbv9eaGlKL7yA=";
    };
  };
  ibm-plex-sans-jp = {
    pname = "ibm-plex-sans-jp";
    version = "@ibm/plex-sans-jp@2.0.0";
    src = fetchurl {
      url = "https://github.com/IBM/plex/releases/download/@ibm/plex-sans-jp@2.0.0/ibm-plex-sans-jp.zip";
      sha256 = "sha256-zmrxxxa/4k0vgSkSnk/qjNS1c4adhBfsTyLqseB9pRA=";
    };
  };
  lazygit = {
    pname = "lazygit";
    version = "v0.54.2";
    src = fetchFromGitHub {
      owner = "jesseduffield";
      repo = "lazygit";
      rev = "v0.54.2";
      fetchSubmodules = false;
      sha256 = "sha256-LfSTbnSyRT1vdrEOs9Ur+0cGAz/pUUEVm8HhfE9VaYo=";
    };
  };
  octocov = {
    pname = "octocov";
    version = "v0.69.0";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "octocov";
      rev = "v0.69.0";
      fetchSubmodules = false;
      sha256 = "sha256-PALv1WPMyUJ/F2tTOaYPqV7WnmHIL4KecJ7PA3IEFdQ=";
    };
  };
  pinact = {
    pname = "pinact";
    version = "v3.4.2";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "pinact";
      rev = "v3.4.2";
      fetchSubmodules = false;
      sha256 = "sha256-O+yLhvkF84uCrgb5MPvk8i/YJ4tLR7YQvBAYbpnxwEM=";
    };
  };
  roots = {
    pname = "roots";
    version = "v0.4.0";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "roots";
      rev = "v0.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-NOUxbYp3dPJiK6ZJ96tyCrdQfeQWMsWfwIpinIQfowU=";
    };
  };
  slack-reminder = {
    pname = "slack-reminder";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "skanehira";
      repo = "slack-reminder";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-1MzRZ8E1wtwThnaXteMkAapX5kLFbEi3ix1csatSFF8=";
    };
  };
  sort-package-json = {
    pname = "sort-package-json";
    version = "v3.4.0";
    src = fetchFromGitHub {
      owner = "keithamus";
      repo = "sort-package-json";
      rev = "v3.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-CnMWuokQistBfvyyPHJTqAlVj+8r4CcZ8VGYRKB1Mmo=";
    };
  };
  vim-startuptime = {
    pname = "vim-startuptime";
    version = "v1.3.2";
    src = fetchFromGitHub {
      owner = "rhysd";
      repo = "vim-startuptime";
      rev = "v1.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-d6AXTWTUawkBCXCvMs3C937qoRUZmy0qCFdSLcWh0BE=";
    };
  };
}
