# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  cc-sdd = {
    pname = "cc-sdd";
    version = "2.1.1";
    src = fetchurl {
      url = "https://registry.npmjs.org/cc-sdd/-/cc-sdd-2.1.1.tgz";
      sha256 = "sha256-0jdKcS5tKEngUVOvHPDF7/nKnsSsDRG3bkR98zyI4rw=";
    };
  };
  ccusage = {
    pname = "ccusage";
    version = "18.0.10";
    src = fetchurl {
      url = "https://registry.npmjs.org/ccusage/-/ccusage-18.0.10.tgz";
      sha256 = "sha256-YMJ8K2LmS0v0HeoRskfULZ7P7rVBpgYJRlDmZjndRiw=";
    };
  };
  claude-code = {
    pname = "claude-code";
    version = "2.1.74";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.74.tgz";
      sha256 = "sha256-3OM+J+r4knZZjp2uUJnhJsJYDOpR62Rx4cN0eICdGBg=";
    };
  };
  disable-checkout-persist-credentials = {
    pname = "disable-checkout-persist-credentials";
    version = "v1.0.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "disable-checkout-persist-credentials";
      rev = "v1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-TEjUf7Uc049g/ZRuKMtWYxod+ZN5dKabDINJgqXxkuk=";
    };
  };
  duckgo = {
    pname = "duckgo";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "sheepla";
      repo = "duckgo";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-+wJexVAKTqEOTyV+PcNcUe5r9Q4MpGx9EnuxiQHq0G0=";
    };
  };
  firge = {
    pname = "firge";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/Firge_v0.3.0.zip";
      sha256 = "sha256-yYb2IQAKZZgGTH4geDIu0CBAr094BNJYAz35SbNbBSc=";
    };
  };
  firge-nerd = {
    pname = "firge-nerd";
    version = "v0.3.0";
    src = fetchurl {
      url = "https://github.com/yuru7/Firge/releases/download/v0.3.0/FirgeNerd_v0.3.0.zip";
      sha256 = "sha256-VM12N4+8UCX0LUQdlcpuwdPsxCcOYQdViED+18BM/k8=";
    };
  };
  gh-dep = {
    pname = "gh-dep";
    version = "v0.11.0";
    src = fetchFromGitHub {
      owner = "jackchuka";
      repo = "gh-dep";
      rev = "v0.11.0";
      fetchSubmodules = false;
      sha256 = "sha256-hg0hwlt1pQZzKLv2HKKeRVclAX1ZTOpMyaqxaHEnnYo=";
    };
  };
  gh-grep = {
    pname = "gh-grep";
    version = "v1.2.5";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "gh-grep";
      rev = "v1.2.5";
      fetchSubmodules = false;
      sha256 = "sha256-xCr68IMrGPIQr+RmAgNdgD+Bw0dQSSi9GcpoDy3q86U=";
    };
  };
  gh-triage = {
    pname = "gh-triage";
    version = "v0.11.0";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "gh-triage";
      rev = "v0.11.0";
      fetchSubmodules = false;
      sha256 = "sha256-aa98DfXxyOuk1Q7yYGKNqYqTa9tuNP57hYwNqIYxPeo=";
    };
  };
  ghalint = {
    pname = "ghalint";
    version = "v1.5.5";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghalint";
      rev = "v1.5.5";
      fetchSubmodules = false;
      sha256 = "sha256-xAXcvvSwcJjdG2BCItBLdsu6vZiID5FmRYYF9PZe1QE=";
    };
  };
  ghatm = {
    pname = "ghatm";
    version = "v1.0.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "ghatm";
      rev = "v1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-MX1VilyfxJ9GKIkDSGeAE007DyjPkWL5W4b08EqAyC4=";
    };
  };
  ghq = {
    pname = "ghq";
    version = "v1.8.1";
    src = fetchFromGitHub {
      owner = "x-motemen";
      repo = "ghq";
      rev = "v1.8.1";
      fetchSubmodules = false;
      sha256 = "sha256-Iw8hu2QtnRgRbSTqtIPDmKbx5FcE2j68VfzP4egbZgY=";
    };
  };
  git-wt = {
    pname = "git-wt";
    version = "v0.25.0";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "git-wt";
      rev = "v0.25.0";
      fetchSubmodules = false;
      sha256 = "sha256-QdyONDVokpOaH5dI5v1rmaymCgIiWZ16h26FAIsAHPc=";
    };
  };
  gitlab-ci-ls = {
    pname = "gitlab-ci-ls";
    version = "1.2.5";
    src = fetchFromGitHub {
      owner = "alesbrelih";
      repo = "gitlab-ci-ls";
      rev = "1.2.5";
      fetchSubmodules = false;
      sha256 = "sha256-Ly4pk+16RCr3r33VrYPTZGUXfUNd5IJHfA+uj7Ef3bk=";
    };
  };
  gitlab-ci-verify = {
    pname = "gitlab-ci-verify";
    version = "v2.8.0";
    src = fetchFromGitHub {
      owner = "timo-reymann";
      repo = "gitlab-ci-verify";
      rev = "v2.8.0";
      fetchSubmodules = false;
      sha256 = "sha256-oB7KXP+RbWLrCke17a31VheouijdSmkIGtbA8JqkZfM=";
    };
  };
  ibm-plex-sans-jp = {
    pname = "ibm-plex-sans-jp";
    version = "@ibm/plex-sans-jp@3.0.0";
    src = fetchurl {
      url = "https://github.com/IBM/plex/releases/download/@ibm/plex-sans-jp@3.0.0/ibm-plex-sans-jp.zip";
      sha256 = "sha256-TBTEFVKTSwvJL8IWp2s2vcePdF377tERX5uLxAXuMU8=";
    };
  };
  lazygit = {
    pname = "lazygit";
    version = "v0.60.0";
    src = fetchFromGitHub {
      owner = "jesseduffield";
      repo = "lazygit";
      rev = "v0.60.0";
      fetchSubmodules = false;
      sha256 = "sha256-BTcm3wF0bXIGsheDUv437aGO2TgU+oKXYqfagW0N8DQ=";
    };
  };
  octocov = {
    pname = "octocov";
    version = "v0.75.3";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "octocov";
      rev = "v0.75.3";
      fetchSubmodules = false;
      sha256 = "sha256-g749rqGMUx3fMZo4cWoXF76ZOLJIvKTOxf1E04F0QdM=";
    };
  };
  pinact = {
    pname = "pinact";
    version = "v3.9.0";
    src = fetchFromGitHub {
      owner = "suzuki-shunsuke";
      repo = "pinact";
      rev = "v3.9.0";
      fetchSubmodules = false;
      sha256 = "sha256-FbqVf7ANY2wDrQCnDp3pVAT0PyMzwNgdusgNkaNAlKk=";
    };
  };
  roots = {
    pname = "roots";
    version = "v0.4.1";
    src = fetchFromGitHub {
      owner = "k1LoW";
      repo = "roots";
      rev = "v0.4.1";
      fetchSubmodules = false;
      sha256 = "sha256-ACMRfWY/lhc3C/KVhuUyS1rgkSHGWPxZrmYt+pXupJI=";
    };
  };
  slack-reminder = {
    pname = "slack-reminder";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "skanehira";
      repo = "slack-reminder";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-1MzRZ8E1wtwThnaXteMkAapX5kLFbEi3ix1csatSFF8=";
    };
  };
  sort-package-json = {
    pname = "sort-package-json";
    version = "v3.6.1";
    src = fetchFromGitHub {
      owner = "keithamus";
      repo = "sort-package-json";
      rev = "v3.6.1";
      fetchSubmodules = false;
      sha256 = "sha256-PGstuD9NNdY8dkrtLLZjGbUJ7fb4U5mPSFygnZcGi9k=";
    };
  };
  vim-startuptime = {
    pname = "vim-startuptime";
    version = "v1.3.2";
    src = fetchFromGitHub {
      owner = "rhysd";
      repo = "vim-startuptime";
      rev = "v1.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-d6AXTWTUawkBCXCvMs3C937qoRUZmy0qCFdSLcWh0BE=";
    };
  };
}
